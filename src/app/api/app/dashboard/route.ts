...
import { cachedResponse, forbiddenError, errorResponse } from "@/shared/lib/api/responses";
 ...
 * Eliminates 5 separate client→server roundtrips and shares common queries
 * (budgetIds, memberId, partnerPrivacy) across all data fetches.
 */
export const GET = withAuthRequired(async (req, context) => {
  ...
  // === SHARED QUERIES (run once, used by all sections) ===
  ...
  const [userMemberId, budgetRow] = await Promise.all([
    getUserMemberIdInBudget(session.user.id, budgetId),
...
async function fetchCommitments(opts: {
  ...
  const { budgetId, year, month, categoryViewCondition, userMemberId } = opts;
